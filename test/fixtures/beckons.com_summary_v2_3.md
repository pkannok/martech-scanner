# MarTech Scan Summary

- **Scanner version:** 0.1.2
- **Report template version:** 2.3
- **Domain:** https://beckons.com
- **Scanned at:** 2026-04-21T20:07:38.779Z
- **Pages scanned:** 6
- **Max pages configured:** 6
- **Consent click enabled:** true

## Vendors detected

- **WordPress** (cms) via network
- **Google Tag Manager** (tag_manager) via network
- **Meta Pixel** (media_pixel) via network
- **Google Analytics** (analytics) via network
- **Google Ads / DoubleClick** (media_pixel) via network
- **Hotjar** (session_replay) via network
- **TikTok Pixel** (media_pixel) via network
- **The Trade Desk** (media_pixel) via network
- **Adobe Analytics / Experience Cloud** (analytics) via network
- **TikTok Pixel** (media_pixel) via script
- **Hotjar** (session_replay) via script
- **Google Ads / DoubleClick** (media_pixel) via script
- **Google Analytics** (analytics) via script
- **Meta Pixel** (media_pixel) via script
- **Google Tag Manager** (tag_manager) via script
- **The Trade Desk** (media_pixel) via script
- **Google Analytics** (analytics) via source_code
- **Google Ads / DoubleClick** (media_pixel) via source_code
- **Meta Pixel** (media_pixel) via source_code
- **TikTok Pixel** (media_pixel) via source_code
- **The Trade Desk** (media_pixel) via source_code
- **Google Tag Manager** (tag_manager) via source_code

## IDs found

- **GTM Container ID:** `GTM-PC3TL92D`
- **GA4 Measurement ID:** `G-G5Y3RBXDG8`
- **Google Ads ID:** `AW-17911345596`
- **TikTok Pixel ID:** `D4T4CDJC77U9L5PIV3O0`
- **The Trade Desk Advertiser ID:** `fypp50u`
- **The Trade Desk Advertiser ID:** `aam`
- **DoubleClick Advertiser ID:** `DC-16510922`
- **Facebook Pixel ID:** `633710700486705`

## Page-level findings

### https://beckons.com
- Status: ok
- HTTP status: 200
- Title: Beckons - Luxury Lodges & Exclusive Travel Experiences
- Network findings: 158
- Third-party scripts: 13
- Cookies captured: 15
- Globals present: dataLayer
- Google source signals: google_tag_manager, dataLayer
- Source-derived IDs: 11
  - GTM Container ID: GTM-PC3TL92D
  - Facebook Pixel ID: 633710700486705
  - GA4 Measurement ID: G-G5Y3RBXDG8
  - Google Ads ID: AW-17911345596
  - DoubleClick Advertiser ID: DC-16510922
  - TikTok Pixel ID: D4T4CDJC77U9L5PIV3O0
  - The Trade Desk Advertiser ID: fypp50u

### https://beckons.com/
- Status: ok
- HTTP status: 200
- Title: Beckons - Luxury Lodges & Exclusive Travel Experiences
- Network findings: 158
- Third-party scripts: 13
- Cookies captured: 15
- Globals present: dataLayer
- Google source signals: google_tag_manager, dataLayer
- Source-derived IDs: 11
  - GTM Container ID: GTM-PC3TL92D
  - Facebook Pixel ID: 633710700486705
  - GA4 Measurement ID: G-G5Y3RBXDG8
  - Google Ads ID: AW-17911345596
  - DoubleClick Advertiser ID: DC-16510922
  - TikTok Pixel ID: D4T4CDJC77U9L5PIV3O0
  - The Trade Desk Advertiser ID: fypp50u

### https://beckons.com/#about
- Status: ok
- HTTP status: 200
- Title: Beckons - Luxury Lodges & Exclusive Travel Experiences
- Network findings: 170
- Third-party scripts: 14
- Cookies captured: 15
- Globals present: dataLayer
- Google source signals: google_tag_manager, dataLayer
- Source-derived IDs: 10
  - GTM Container ID: GTM-PC3TL92D
  - GA4 Measurement ID: G-G5Y3RBXDG8
  - Google Ads ID: AW-17911345596
  - DoubleClick Advertiser ID: DC-16510922
  - Facebook Pixel ID: 633710700486705
  - TikTok Pixel ID: D4T4CDJC77U9L5PIV3O0
  - The Trade Desk Advertiser ID: fypp50u

### https://beckons.com/#lodges
- Status: ok
- HTTP status: 200
- Title: Beckons - Luxury Lodges & Exclusive Travel Experiences
- Network findings: 167
- Third-party scripts: 13
- Cookies captured: 15
- Globals present: dataLayer
- Google source signals: google_tag_manager, dataLayer
- Source-derived IDs: 10
  - GTM Container ID: GTM-PC3TL92D
  - GA4 Measurement ID: G-G5Y3RBXDG8
  - Google Ads ID: AW-17911345596
  - DoubleClick Advertiser ID: DC-16510922
  - Facebook Pixel ID: 633710700486705
  - TikTok Pixel ID: D4T4CDJC77U9L5PIV3O0
  - The Trade Desk Advertiser ID: fypp50u

### https://beckons.com/about/
- Status: ok
- HTTP status: 200
- Title: About Our Collection of Luxury Wilderness Lodges | Beckons
- Network findings: 108
- Third-party scripts: 13
- Cookies captured: 15
- Globals present: dataLayer
- Google source signals: google_tag_manager, dataLayer
- Source-derived IDs: 11
  - GTM Container ID: GTM-PC3TL92D
  - Facebook Pixel ID: 633710700486705
  - GA4 Measurement ID: G-G5Y3RBXDG8
  - Google Ads ID: AW-17911345596
  - DoubleClick Advertiser ID: DC-16510922
  - TikTok Pixel ID: D4T4CDJC77U9L5PIV3O0
  - The Trade Desk Advertiser ID: fypp50u

### https://beckons.com/contact-us/
- Status: ok
- HTTP status: 200
- Title: Contact Our Travel Specialists | Beckons
- Network findings: 95
- Third-party scripts: 13
- Cookies captured: 16
- Globals present: dataLayer
- Google source signals: google_tag_manager, dataLayer
- Source-derived IDs: 11
  - GTM Container ID: GTM-PC3TL92D
  - Facebook Pixel ID: 633710700486705
  - GA4 Measurement ID: G-G5Y3RBXDG8
  - Google Ads ID: AW-17911345596
  - DoubleClick Advertiser ID: DC-16510922
  - TikTok Pixel ID: D4T4CDJC77U9L5PIV3O0
  - The Trade Desk Advertiser ID: fypp50u

## Caveats

- Each page is scanned in a fresh browser context to reduce cache/state contamination.
- IDs may come from network traffic, script URLs, HTML, inline scripts, iframe URLs, noscript blocks, and request bodies.
- This scanner still only observes what is available through public browser activity.
- It may miss deferred tags, server-side tagging, login-gated tooling, and non-fired rules.
